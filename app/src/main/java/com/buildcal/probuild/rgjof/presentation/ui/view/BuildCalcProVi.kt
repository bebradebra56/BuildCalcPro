...
class BuildCalcProVi(
    private val buildCalcProContext: Context,
    private val buildCalcProCallback: BuildCalcProCallBack,
    private val buildCalcProWindow: Window
) : WebView(buildCalcProContext) {
    private var buildCalcProFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun buildCalcProSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.buildCalcProFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(buildCalcProContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        buildCalcProContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(buildCalcProContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true && url?.contains("https://web.team-s.club") != true) {
                    BuildCalcProApplication.buildCalcProInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(BuildCalcProApplication.BUILD_CALC_PRO_MAIN_TAG, "onPageFinished : ${BuildCalcProApplication.buildCalcProInputMode}")
                    buildCalcProWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    buildCalcProEnableStableInputScroll()
                } else {
                    BuildCalcProApplication.buildCalcProInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(BuildCalcProApplication.BUILD_CALC_PRO_MAIN_TAG, "onPageFinished : ${BuildCalcProApplication.buildCalcProInputMode}")
                    buildCalcProWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                buildCalcProFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                buildCalcProHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun buildCalcProFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun buildCalcProEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun buildCalcProHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = BuildCalcProVi(buildCalcProContext, buildCalcProCallback, buildCalcProWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            buildCalcProCallback.buildCalcProHandleCreateWebWindowRequest(windowWebView)
        }
    }
